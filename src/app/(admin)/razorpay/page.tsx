...
export default async function RazorpayDashboard({ searchParams }: PageProps) {
  ...
  const [totalCount, transactions, successSummary] = await Promise.all([
    prisma.transaction.count({ where: { gateway: 'RAZORPAY' } }),
    ...
    prisma.transaction.aggregate({
      where: { gateway: 'RAZORPAY', status: 'SUCCESS' },
      _sum: { amount: true },
      _count: { id: true }
    })
  ]);
  ...
  return (
    <div className="p-6 pb-20">
      <div className="mb-8">
        <h1 className="text-3xl font-bold font-['Outfit'] text-white tracking-tight">Razorpay Transactions</h1>
        <p className="text-gray-400 mt-2">Monitor Razorpay payment logs, verification status, and gateway identifiers.</p>
      </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 bg-[#1c1f2e] border-t border-[#2a2d3e]">
            ...
            <div className="flex gap-2">
              {currentPage > 1 && (
                <Link
                  href={`/razorpay?page=${currentPage - 1}`}
