...
export const GameCard: React.FC<GameCardProps> = ({ game, onJoin }) => {
  const sportName = typeof game.sport === 'object' ? game.sport?.name : game.sport;
  ...
  const currentPlayers = game.participantCount || game.participants?.length || game.joinedCount || 0;
    ...
    <div className="bg-[var(--play-surface)] rounded-[var(--play-radius-md)] border border-[var(--play-border)] p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="mb-4">
        <div className="flex justify-between items-end mb-1.5">
          ...
          <div className="text-xs font-medium text-[var(--play-text-muted)]">
            <span className="text-[var(--play-text)]">{currentPlayers}</span> / {totalPlayers} joined
          </div>
        </div>
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t border-[var(--play-border)]">
        <div className="flex items-center gap-2.5">
          {game.hostAvatar ? (
            <img src={game.hostAvatar} alt={hostNameStr} className="w-8 h-8 rounded-full object-cover bg-[var(--play-surface-alt)]" />
          ) : (
          ...
          <div>
            ...
            <div className="text-sm font-medium text-[var(--play-text)]">{hostNameStr}</div>
          </div>
        </div>
        <button
